...
exports.bookTourPaymentAsync = async (req, res, next) => {
    try {
        ...
        var discount = await DISCOUNT.findOne({ code: req.value.body.codediscount, idTour: req.value.body.idTour });
        ...
        if (req.value.body.typePayment == defaultPayment.VNPay) {
            if (req.value.body.codediscount == "") {
                ...
                res.status(200).json({ code: '00', data: vnpUrl, message: "Successfully Create Payment!" });
            }
            else {
            ...
            if (req.value.body.codediscount == '') {
                ...
                var finalpayment = ((tour.payment - ((tour.payment * discount.discount) / 100)) / 23000).toFixed(1);
                paymentMethod(
                    tour.name,
                    finalpayment,
                    userId,
    ...
    paypal.payment.execute(
        ...
        async function (error, payment) {
            if (error) {
                ...
                const booktour = await BOOKTOUR.findOne({ idUser: idUser, idTour: idTour, status: defaultBookTour.AWAIT });
                if (booktour != null) {
                    var resultBookTour = await BOOKTOUR.findOneAndUpdate({ idUser: idUser, idTour: idTour, status: defaultBookTour.AWAIT },
                        {
                            idPay: paymentId,
